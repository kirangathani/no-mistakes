...
- When the reviewer's final output is rejected, reruns the review with the same prompt plus a note quoting the validation error, up to three attempts in total.
...
- Records an observation whose remedy is documentation or comment wording in a `doc_report`, and one the project's lint command, formatter, or type checker would catch deterministically in a `lint_report`, instead of reporting it as a finding at any severity. Neither report parks the run or starts a fix round; the Document and Lint steps consume them, and their changes are never re-reviewed. Defects, security and privacy, intent departures, the Simplification pass, and a type hint or comment that is WRONG in a way the tool accepts and would mislead a caller all remain findings. See [Review handoff reports](/no-mistakes/concepts/review-handoff-reports/)
- Returns the full report each round (the last one wins, with no retraction lines), persists both with the run, and logs every note, so `no-mistakes axi status` renders them under the review step, `no-mistakes axi logs --step review` shows them, and the PR body's Pipeline section records them beside the review conversation
- Emits each substantiated larger question to the run's [review conversation](/no-mistakes/concepts/review-conversation/) the moment it has one, keeps reviewing other areas while a question is open, and re-reads answers at its own checkpoints so an answer that arrives mid-pass can redirect the rest of it; minor questions it decides itself, as before
- Parks in waiting-on-answers when a pass ends with an unanswered question, carrying one `ask-user` warning per open question; the agent turn has already ended, so the parked wait consumes no `review_agent_timeout` and is accounted as park time. `no-mistakes axi answer --question <id> --answer "..."` records each answer, and once none are open the same reviewer session is resumed to finish the pass - approve and fix are not how a question is answered
- Reuses a reviewer session across exactly one review PASS (the asking turn plus the finalize turn that receives its answers) and never across a code change: the session identity is dropped before any fix round, so the rereview that certifies a fix round never resumes the session whose findings prescribed those fixes, and the finalize prompt is the complete review prompt plus the answers so a failed resume degrades to a cold review rather than a meaningless one; the rereview prompt additionally reframes fix-round changes as pipeline-authored code to review under the same adversarial standard as the author's changes, with prior findings, fix summaries, and same-round tests treated as claims rather than evidence; when the defects it finds sit in prior-fix-round code that exceeds what the original finding required, it reports a single `ask-user` finding recommending that round be reverted to the minimal fix instead of filing further repairs on that code
...
- Receives the most recent other run's review rounds on the same branch as a superseded-rounds section, so a run that replaced a parked one still sees what was found and answered; it carries no pipeline-authored provenance framing, because the author wrote those commits
...
- After the baseline passes, fails, or is absent, consults the **diff-class gate** to decide whether the live-evidence agent runs at all. The gate never touches the configured command above, which runs on every run regardless. It classifies the run's diff - the merge base with the trusted default branch through the run's head, the same base the Rebase step establishes - with trusted [`test.non_product_paths`](/no-mistakes/reference/repo-config/#testnon_product_paths), and skips the evidence turn in exactly two cases:
  - **No product file in the diff.** There is no live-drivable surface by construction, so the step records the `no-surface` verdict and marks it automatic with the reason `no product file in diff`. Unlike the agent's own `no-surface`, an automatic one does **not** park: the classification is mechanical, so there is nothing for a human to decide and the run proceeds to Document. A failing `commands.test` still parks the step on its own finding.
  - **Product files unchanged since an earlier `go` verdict on the same branch.** The most recent `go` verdict recorded for this branch is re-recorded with a pointer to the run that earned it and its evidence directory, and the reason `product files unchanged since <short sha>`. Nothing else is reusable: not another branch's verdict, not a `no-go`, `inconclusive`, or `no-surface` one, and not any verdict while this run's configured `commands.test` is red.
- Every other run invokes the evidence agent, and so does any failure to establish either case above: the gate fails open to a full evidence turn rather than guessing.
- The step records which path it took in its findings (`evidence_source` plus a one-line `evidence_reason`), so `no-mistakes axi` reports it as `live_evidence`, the step log names it, and the PR body's Testing section appends it to the live-validation line. A run recorded before the gate existed carries neither field and renders exactly as before.
- When the evidence agent does run, it always drives scenarios. The agent derives a proportionate list of named end-user scenarios from user intent and the change, stands up the real product, and drives each scenario end to end. Repository-specific startup guidance can be supplied through trusted [`test.instructions`](/no-mistakes/reference/repo-config/#testinstructions).
...
**Approval:** a `no-go` verdict adds an auto-fixable error and parks the step; an `inconclusive` verdict adds an `ask-user` warning and parks for a decision; a `no-surface` verdict adds an `ask-user` warning asking whether to proceed without live validation and parks for a decision, unless the diff-class gate reached it automatically, which never parks. An `untested` scenario never parks by itself. Other test findings follow their `action`: `ask-user` pauses for approval, `auto-fix` stays eligible for the fix loop, and `no-op` is informational only.
...
- Owns comment, test-name, and descriptive log or summary string WORDING in code and workflow files as well as prose documentation, which is what lets the review step hand those observations over instead of parking on them
- Receives the review step's [`doc_report`](/no-mistakes/concepts/review-handoff-reports/) (and, in the combined housekeeping pass, the `lint_report` too), must act on every note or say why not, and reports one `applied_notes` entry per note; a note it never mentions is recorded as unaddressed rather than assumed done
...
- Consumes the review step's [`lint_report`](/no-mistakes/concepts/review-handoff-reports/) on every path: when the configured lint command PASSES and the report is non-empty, it still spends one agent pass confirming and fixing those notes, because a configured command says nothing about the checks it does not run (a type check, for example); when the command fails, the report rides the fix round's prompt; when no command is configured, it rides the agent lint pass or the combined housekeeping pass. It reports one `applied_notes` entry per note, and a note it cannot resolve comes back as an ordinary lint finding under the usual gate. A passing command with an empty report still runs no agent at all
