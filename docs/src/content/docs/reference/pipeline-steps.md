...
- When the reviewer's final output is rejected, reruns the review with the same prompt plus a note quoting the validation error, up to three attempts in total; the rerun is a fresh, session-free review, except with the conversation on, where it stays in that pass's reviewer session, which already holds the work it just did
...
- Runs every review turn - the initial review and every full rereview - as a fresh, session-free invocation, unless trusted [`review.conversation: true`](/no-mistakes/reference/repo-config/#reviewconversation) is on, where one session spans exactly one review PASS (the asking turn plus the finalize turn that receives its answers) and never a code change: either way the rereview that certifies a fix round never resumes the session whose findings prescribed those fixes, because the session identity is dropped before any fix round, and the finalize prompt is the complete review prompt plus the answers so a failed resume degrades to a cold review rather than a meaningless one; the rereview prompt additionally reframes fix-round changes as pipeline-authored code to review under the same adversarial standard as the author's changes, with prior findings, fix summaries, and same-round tests treated as claims rather than evidence; when the defects it finds sit in prior-fix-round code that exceeds what the original finding required, it reports a single `ask-user` finding recommending that round be reverted to the minimal fix instead of filing further repairs on that code
- With trusted [`review.conversation: true`](/no-mistakes/reference/repo-config/#reviewconversation) - off by default, and the gate for every conversation bullet that follows - emits each substantiated larger question to the run's [review conversation](/no-mistakes/concepts/review-conversation/) the moment it has one, keeps reviewing other areas while a question is open, and re-reads answers at its own checkpoints so an answer that arrives mid-pass can redirect the rest of it; minor questions it decides itself, as before
...
- Emits a single `ask-user` warning (`review-questions-unreadable`) INSTEAD of any per-question row when the reviewer's question history could not be read in full, whatever remains open, because answers are refused for such a conversation and every per-question row would instruct an `axi answer` the daemon is guaranteed to refuse; the warning names the ids and count of the questions that were still open in the part of the history that was read and points at the run's `questions.ndjson` as the full record. It deliberately carries no review-question category, so nothing resumes a reviewer off a history it could not read, and both automatic resolvers (`axi --yes` and the TUI's yolo) stand aside on it by finding ID while a human's own approve, fix, or skip stays allowed
