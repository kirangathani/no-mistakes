...
- Reuses a reviewer session across exactly one review PASS (the asking turn plus the finalize turn that receives its answers) and never across a code change - with the conversation off, no review turn keeps a session at all: the session identity is dropped before any fix round, so the rereview that certifies a fix round never resumes the session whose findings prescribed those fixes, and the finalize prompt is the complete review prompt plus the answers so a failed resume degrades to a cold review rather than a meaningless one; the rereview prompt additionally reframes fix-round changes as pipeline-authored code to review under the same adversarial standard as the author's changes, with prior findings, fix summaries, and same-round tests treated as claims rather than evidence; when the defects it finds sit in prior-fix-round code that exceeds what the original finding required, it reports a single `ask-user` finding recommending that round be reverted to the minimal fix instead of filing further repairs on that code
