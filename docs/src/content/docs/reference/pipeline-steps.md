...
- After the baseline passes, fails, or is absent, invokes the evidence agent, unless the repository opted into the **diff-class gate** with trusted [`test.evidence_gate: diff-class`](/no-mistakes/reference/repo-config/#testevidence_gate). The gate is off by default, and with it off the agent runs on every run and the step records no evidence source at all.
- When the gate is on, it decides whether the live-evidence agent runs. It never touches the configured command above, which runs on every run regardless. It classifies the run's diff - the merge base with the repository's default branch through the run's head, the same base the Review and Document steps read; a repository that sets `pr.base_branch` elsewhere gets a wider diff here, which only ever fails open to running the agent - with trusted [`test.non_product_paths`](/no-mistakes/reference/repo-config/#testnon_product_paths), and skips the evidence turn in exactly two cases:
  ...
  - **Product files unchanged since this branch's newest `go` verdict, earned under the same intent.** The branch's most recently completed Test verdict is re-recorded, and only when that newest verdict is `go`, with a pointer to the run that earned it and the reason `product files unchanged since <short sha>`. The reused verdict stays recorded against the head its scenarios were actually driven at, never restamped onto this run's head, so it claims only that the product behavior those scenarios proved has not changed - not that anything ran here. The pointer names the run, not a directory on the daemon host. The originating run's evidence artifacts are carried forward and its evidence directory is copied into this run's before publication, with each carried artifact path rebased onto this run's directory, so the citations resolve and the verdict is published with evidence a reviewer can open. Only regular files and directories are carried: a symlink in an evidence directory is never reproduced and never cited, because that directory is published. A verdict that evidenced its scenarios with commands rather than files has nothing to copy and is unaffected. If the copy is not possible - the originating directory has aged out under [`test.evidence.retention`](/no-mistakes/reference/global-config/#testevidence), or nothing was recorded - the scenarios are dropped along with their artifacts rather than citing files this PR does not carry; the verdict line and the reuse reason still name the run that holds them. Only the newest verdict is ever consulted, so a later `no-go`, `inconclusive`, or `no-surface` on the branch blocks reuse of an earlier `go` by construction. Nothing else is reusable either: not another branch's verdict, not any verdict while this run's configured `commands.test` is red, not any verdict during a Test fix round - that round exists because this run's own evidence reported a problem, and every earlier verdict on the branch predates it - and not a verdict earned under a different [user intent](/no-mistakes/reference/pipeline-steps/#intent) than this run carries - the evidence turn derives its scenarios from that intent, so a verdict says nothing about acceptance criteria it never exercised. An absent intent on either side counts as a difference, so it too runs the agent. When the verdict being reused was itself a reuse, the pointer names the run that actually drove the agent rather than the intermediate run, which holds no artifacts; that originating run is recorded on the findings rather than parsed back out of the reason text.
...
- With the gate on, the step records which path it took in its findings (`evidence_source` plus a one-line `evidence_reason`), so `no-mistakes axi` reports it as `live_evidence`, the step log names it, and the PR body's Testing section appends it to the live-validation line. A run with the gate off, and any run recorded before the gate existed, carries neither field and renders exactly as before.
...
**Approval:** a `no-go` verdict adds an auto-fixable error and parks the step; an `inconclusive` verdict adds an `ask-user` warning and parks for a decision; a `no-surface` verdict adds an `ask-user` warning asking whether to proceed without live validation and parks for a decision, unless the opted-in diff-class gate reached it automatically, which never parks. An `untested` scenario never parks by itself. Other test findings follow their `action`: `ask-user` pauses for approval, `auto-fix` stays eligible for the fix loop, and `no-op` is informational only.
...
When the Test step validated the same `head_sha`, the payload also includes `live_validation` with `verdict`, `live` (the number of scenarios driven live), `total`, and `source`. The field is omitted for pre-contract findings and whenever a later Document, Lint, Push, or repair commit changes the head without validating that new commit. Consumers therefore never receive a previous head's live-validation verdict as a claim about the current head. `source` names who produced the verdict (`agent`, `reused`, or `no-product-change`) and is itself omitted when the diff-class evidence gate is off or the run predates it. A **reused** verdict is deliberately recorded against the head its scenarios were driven at rather than the current head, so `live_validation` is omitted entirely for the head being published: a run the evidence agent never drove makes no live-validation claim, and the verdict still reaches a reader through the Testing section with its provenance. `source` is what distinguishes an agent turn from an automatic `no-product-change` one, both of which do belong to the published head.
