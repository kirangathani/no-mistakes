...
- When the gate is on, it decides whether the live-evidence agent runs. It never touches the configured command above, which runs on every run regardless. It classifies the run's diff - the merge base with the repository's default branch through the run's head, the same base the Review and Document steps read; a repository that sets `pr.base_branch` elsewhere gets a wider diff here, which only ever fails open to running the agent - with trusted [`test.non_product_paths`](/no-mistakes/reference/repo-config/#testnon_product_paths), and skips the evidence turn in exactly two cases:
  ...
  - **Product files unchanged since this branch's newest `go` verdict, earned under the same intent.** The branch's most recently completed Test verdict is re-recorded, and only when that newest verdict is `go`, with a pointer to the run that earned it and the reason `product files unchanged since <short sha>`. The reused verdict stays recorded against the head its scenarios were actually driven at, never restamped onto this run's head, so it claims only that the product behavior those scenarios proved has not changed - not that anything ran here. The pointer names the run, not a directory on the daemon host. The originating run's evidence artifacts are carried forward and its evidence directory is copied into this run's before publication, so the scenario table's citations still resolve and the verdict is published with evidence a reviewer can open. If that copy is not possible - the originating directory has aged out under [`test.evidence.retention`](/no-mistakes/reference/global-config/#testevidence), or nothing was recorded - the artifact list and the scenario table are dropped together rather than citing files this PR does not carry; the verdict line and the reuse reason still name the run that holds them. Only the newest verdict is ever consulted, so a later `no-go`, `inconclusive`, or `no-surface` on the branch blocks reuse of an earlier `go` by construction. Nothing else is reusable either: not another branch's verdict, not any verdict while this run's configured `commands.test` is red, and not a verdict earned under a different [user intent](/no-mistakes/reference/pipeline-steps/#intent) than this run carries - the evidence turn derives its scenarios from that intent, so a verdict says nothing about acceptance criteria it never exercised. An absent intent on either side counts as a difference, so it too runs the agent. When the verdict being reused was itself a reuse, the pointer names the run that actually drove the agent rather than the intermediate run, which holds no artifacts; that originating run is recorded on the findings rather than parsed back out of the reason text.
